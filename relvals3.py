...
for sel in [30, 3]:
    ...
    fminasi = []
    for _ in tqdm(range(100), desc=f"running toys for fit with {sel}"):
        asimov = binomsp.rvs(size=int(ntot_ae), n=nmax, p=pval)
        nll_asimov = create_loss(asimov[asimov < sel], nmax)
        p.set_value(np.random.uniform(0.005, 0.05))
        result_asimov = minimizer.minimize(nll_asimov, p).update_params()
        fminasi.append(result_asimov.fmin)
    fminasi = np.array(fminasi)

    n_above = np.sum(fminasi > result.fmin)
    pvalue_toys = n_above / len(fminasi) / 2  # we're only interested in fluctuations where it first overshoots, under and then over
    pvalue_toys_unc = pvalue_toys / n_above ** 0.5
    sigma_toys = (scipy.stats.chi2(1).isf(pvalue_toys)) ** 0.5
    print(f"p value from toys: {pvalue_toys:.3g} +- {pvalue_toys_unc:.2g}, sigma={sigma_toys:.3g}")
    result_asimov.hesse()
    ...
    deltaLL = result.fmin - np.mean(fminasi)
    nsigma = (deltaLL * 2) ** 0.5
    pvalue = scipy.stats.chi2.sf(nsigma ** 2, 1)
    print(f"asimov: {np.mean(fminasi)} +- {np.std(fminasi)}, fmin data: {result.fmin}")
